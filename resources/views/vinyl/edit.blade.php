<x-layout title="Vinyl: {{ $vinyl->name }} aanpassen">
    <div class="p-20">
        ...
        <div class="mt-2">
            <form action="{{ route('vinyl.update', $vinyl->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" min="1" max="500" step="1" name="stock" id="stock" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('stock', $vinyl->stock) }}" required>
                </div>
                <div class="mb-4">
                    <label for="price" class="block text-gray-700 text-sm font-bold mb-2">Prijs:</label>
                    <input type="number" min="1" max="500" step="any" name="price" id="price" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('price', $vinyl->price) }}" required>
                </div>
